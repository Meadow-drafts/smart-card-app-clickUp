        ...
        <nav className="bg-gray-100 fixed w-full z-20 top-0 left-0 border-b shadow-md">
            <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
                <a href="https://flowbite.com/" className="flex items-center">
                    <img src={logo} className="h-12 mr-3" alt="app Logo" />
                    <span className="self-center text-2xl font-semibold whitespace-nowra">AppName</span>
                </a>
                <div className="flex  md:order-3 ">
                    ...
                    <button onClick={()=> setToggle((prev)=> !prev)} data-collapse-toggle="navbar-sticky" type="button" className="inline-flex items-center p-3 text-sm text-gray-500 rounded-lg md:hidden visible hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-gray-200 dark:text-gray-400 dark:hover:bg-gray-700 dark:focus:ring-gray-600" aria-controls="navbar-sticky" aria-expanded="false">
                    ...
                    <ul className="flex flex-col p-4 md:p-0 mt-4 font-medium border border-gray-100 rounded-lg bg-gray-100 md:flex-row md:space-x-8 md:mt-0 md:border-0">
                        {
                            navLinks.map((nav, index) => (
                                <li>
                                    <a href={`#${nav.id}`} className="block py-2 pl-3 pr-4 text-gray-900 hover:text-gray-400" aria-current="page">{nav.title}</a>
                                </li>
                            ))
                        }
